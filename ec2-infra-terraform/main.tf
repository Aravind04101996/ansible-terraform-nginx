...
resource "null_resource" "execute_ansible_target" {
  ...
  provisioner "local-exec" {
   command =  <<EOT
      ...
      ansible-playbook ../ansible-nginx-playbook/webserver_playbook.yml --extra-vars webserver_playbook_variables.yml -u ec2-user --private-key ${local_file.ec2_private_key.filename} -i inventory.txt 
    EOT
  }
}